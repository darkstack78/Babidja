  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (user.role !== 'SUPER_ADMIN') {
        setFormError("Ce compte n'a pas les droits d'administration de la plateforme.")
        return
      }
    ...
    <div className="relative grid min-h-screen place-items-center overflow-hidden bg-gradient-to-br from-primary/30 via-primary/10 to-primary/40 px-4">
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl ring-1 ring-primary/20 relative z-10">
        ...
        <p className="mt-1 text-center text-xs font-semibold text-primary">
          Babydja Platform Control
        </p>

        <div className="mx-auto mt-6 w-fit rounded-2xl bg-primary px-6 py-3 text-center text-sm font-extrabold uppercase tracking-wide text-white shadow-sm">
          Super Administrateur
        </div>

        <form className="mt-6 flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
          <Input type="email" placeholder="E-mail administrateur" {...register('email')} error={errors.email?.message} />
